...
*Setup instructions for a gitian build of Qodex Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Qodex
...
to the Qodex GitHub release page.
...
- [Building Qodex Core](#building-qodex-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/qodex-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/qodex-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/qodex-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/qodex-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/qodex-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/qodex-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/forking-altcoins/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@qodex.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/qodex-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/qodex-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/qodex-osx-*-build.assert
```
