...
The build script we'll be using is contained in the Qodex github repository ([contrib/gitian-build.py](https://github.com/forking-altcoins/qodex/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the Qodex repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/forking-altcoins/Qodex/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, qodex-detached-sigs, and qodex GitHub repos.
