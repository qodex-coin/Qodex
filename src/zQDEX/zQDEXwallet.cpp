// Copyright (c) 2017-2019 The Qodex developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zQDEXwallet.h"
#include "main.h"
#include "txdb.h"
...
CzQDEXWallet::CzQDEXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zQDEX seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZQDEXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZQDEXSeed_deprecated()) {
                    LogPrintf("%s: Updated zQDEX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zQDEX seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zQDEX wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzQDEXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZQDEXCount(nCountLastUsed);
    else if (!walletdb.ReadZQDEXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzQDEXWallet::Lock()
{
    seedMaster = 0;
}

void CzQDEXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzQDEXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZQDEX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzQDEXWallet::LoadMintPoolFromDB()
{
...
void CzQDEXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzQDEXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzQDEXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zQDEXTracker->HasPubcoinHash(pMint.first)) {
...
bool CzQDEXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZQDEX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zQDEXTracker which also adds to database
    pwalletMain->zQDEXTracker->Add(dMint, true);
...
void CzQDEXWallet::SeedToZQDEX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzQDEXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzQDEXWallet::GenerateDeterministicZQDEX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzQDEXWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzQDEXWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzQDEXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
