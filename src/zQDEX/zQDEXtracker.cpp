// Copyright (c) 2018-2020 The Qodex developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zQDEX/deterministicmint.h>
#include "zQDEXtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzQDEXTracker::CzQDEXTracker(std::string strWalletFile)
{
...
CzQDEXTracker::~CzQDEXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzQDEXTracker::Init()
{
...
bool CzQDEXTracker::Archive(CMintMeta& meta)
{
...
bool CzQDEXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzQDEXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzQDEXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzQDEXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzQDEXTracker::GetSerialHashes()
{
...
CAmount CzQDEXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzQDEXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzQDEXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzQDEXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzQDEXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzQDEXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzQDEXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzQDEXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzQDEXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzQDEXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzQDEXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzQDEXWallet* zQDEXWallet)
{
    bool iszQDEXWalletInitialized = (NULL != zQDEXWallet);
    CMintMeta meta;
    ...
    if (! iszQDEXWalletInitialized)
        zQDEXWallet = new CzQDEXWallet(strWalletFile);
    meta.isSeedCorrect = zQDEXWallet->CheckSeed(dMint);
    if (! iszQDEXWalletInitialized)
        delete zQDEXWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzQDEXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzQDEXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzQDEXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzQDEXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzQDEXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzQDEXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzQDEXWallet* zQDEXWallet = new CzQDEXWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zQDEXWallet);
        }
        delete zQDEXWallet;
        LogPrint("zero", "%s: added %d dzQDEX from DB\n", __func__, listDeterministicDB.size());
    }
