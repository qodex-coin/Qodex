...
#include "qt/qodex/send.h"
#include "qt/qodex/forms/ui_send.h"
#include "qt/qodex/addnewcontactdialog.h"
#include "qt/qodex/qtutils.h"
#include "qt/qodex/sendchangeaddressdialog.h"
#include "qt/qodex/optionbutton.h"
#include "qt/qodex/sendconfirmdialog.h"
#include "qt/qodex/myaddressrow.h"
#include "qt/qodex/guitransactionsutils.h"
...
#include "zQDEXcontroldialog.h"

SendWidget::SendWidget(QodexGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (QDEX)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("Qodex address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 QDEX");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_QDEX : AskPassphraseDialog::Context::Send_zQDEX, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zQDEX is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zQDEX selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zQDEX transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZQDEX_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zQDEX require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zQDEX transaction
        ...
        emit message("zQDEX transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isQDEX){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zQDEX in your balance to select."));
        }
    }
}
...
void SendWidget::onQDEXSelected(bool _isQDEX){
    isQDEX = _isQDEX;
    setCssProperty(coinIcon, _isQDEX ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
