...
#include "qt/qodex/dashboardwidget.h"
#include "qt/qodex/forms/ui_dashboardwidget.h"
#include "qt/qodex/sendconfirmdialog.h"
#include "qt/qodex/txrow.h"
...
DashboardWidget::DashboardWidget(QodexGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of QDEX staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarePiv, "square-chart-piv");
    setCssProperty(ui->labelSquarezPiv, "square-chart-zQDEX");
    ...
    ui->labelAmountPiv->setText("0 QDEX");
    ...
    ui->btnHowTo->setText(tr("How to get QDEX"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("QDEX")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPiv = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZQDEX;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair QDEX, zQDEX
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            zQDEX = (pair.second != 0) ? pair.second / 100000000 : 0;
        ...
        chartData->valueszPiv.append(zQDEX);

        int max = std::max(piv, zQDEX);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("QDEX");
    set1 = new QBarSet("zQDEX");
