...
#include "qt/qodex/privacywidget.h"
#include "qt/qodex/forms/ui_privacywidget.h"
#include "qt/qodex/qtutils.h"
#include "guiutil.h"
#include "qt/qodex/denomgenerationdialog.h"
#include "qt/qodex/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(QodexGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zQDEX anonymizes your QDEX by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zQDEX or convert back to QDEX"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 QDEX ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zQDEX");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zQDEX");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zQDEX");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zQDEX");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zQDEX");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zQDEX");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zQDEX");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zQDEX");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zQDEX Movements"));
    ...
    // Only Convert to QDEX enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzQDEX->setTitleClassAndText("btn-title-grey", "Total 0 zQDEX");
    ui->btnTotalzQDEX->setSubTitleClassAndText("text-subtitle", "Show denominations of zQDEX owned.");
    ui->btnTotalzQDEX->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select QDEX outputs to mint into zQDEX.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zQDEX");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzQDEX, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zQDEX");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of QDEX to mint into zQDEX"));
    }else{
        btnText = tr("Convert back to QDEX");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zQDEX to convert back into QDEX"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzQDEX->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzQDEX->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zQDEX is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zQDEX, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zQDEX").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zQDEX minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zQDEX converted back to QDEX"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any QDEX to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zQDEX </b>";
    ...
    ui->btnTotalzQDEX->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
