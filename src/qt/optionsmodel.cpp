...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::QDEX);
    ...
    if (settings.contains("nAnonymizeQodexAmount") || reset)
        SoftSetArg("-anonymizeqodexamount", settings.value("nAnonymizeQodexAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in qodex.cpp during shotdown to also remove the window positions
