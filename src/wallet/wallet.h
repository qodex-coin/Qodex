...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 QDEX at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zQDEX send
enum ZerocoinSpendStatus {
    ZQDEX_SPEND_OKAY = 0,                            // No error
    ZQDEX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZQDEX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZQDEX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZQDEX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZQDEX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZQDEX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZQDEX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZQDEX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZQDEX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZQDEX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZQDEX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZQDEX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZQDEX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZQDEX_SPENT_USED_ZQDEX = 14,                      // Coin has already been spend
    ZQDEX_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZQDEX_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZQDEXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzQDEXAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzQDEXTracker> zQDEXTracker;
    ...
    void setZWallet(CzQDEXWallet* zwallet);
    CzQDEXWallet* getZWallet();
    ...
    /// Get 10000 QDEX output and keys which can be used for the Masternode
    ...
    /** zQDEX reset */
    boost::signals2::signal<void()> NotifyzQDEXReset;
