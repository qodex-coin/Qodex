...
public:
    ...
    bool WriteZQDEXSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZQDEXSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZQDEXSeed_deprecated(uint256& seed);
    bool EraseZQDEXSeed();
    bool EraseZQDEXSeed_deprecated();

    bool WriteZQDEXCount(const uint32_t& nCount);
    bool ReadZQDEXCount(uint32_t& nCount);
