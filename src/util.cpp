...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "qodex" is a composite category enabling all Qodex-related debug output
            if (ptrCategory->count(std::string("qodex"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Qodex
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Qodex
// Mac: ~/Library/Application Support/Qodex
// Unix: ~/.qodex
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Qodex";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "qodex.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty qodex.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override qodex.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "qodexd.pid"));
