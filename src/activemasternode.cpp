...
//
// Bootup the Masternode, look for a 10000 Qodex input and register on the network
//
...
std::vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    for (const COutput& out : vCoins) {
        if (IsValidCollateral(out.tx->vout[out.i].nValue, chainActive.Height())) { //exactly
