...
from test_framework.test_framework import QodexTestFramework
...
class ZerocoinSpendTest(QodexTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zQDEX_bal, piv_bal):
            zQDEX_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zQDEX_bal)
            piv_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], piv_bal)
            return zQDEX_bal, piv_bal
        ...
        zQDEX_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zQDEX_balance, 6666)
        ...
        zQDEX_balance, balance = check_balances(denom_0, zQDEX_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zQDEX_balance, balance = check_balances(denom_2, zQDEX_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zQDEX_balance, balance = check_balances(denom_3, zQDEX_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zQDEX",
        ...
        _, _ = check_balances(denom_1, zQDEX_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
